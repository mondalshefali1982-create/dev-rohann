...
// 13. FORM SUBMISSION VIA FORMSUBMIT AJAX (SILENT BACKGROUND DISPATCH)
if (contactForm) {
  contactForm.addEventListener('submit', (e) => {
    ...
    fetch("https://formsubmit.co/ajax/mondalrohan741@gmail.com", {
      method: "POST",
      headers: { 
      ...
      body: JSON.stringify({
        ...
        subject: subject,
        message: message,
        _captcha: "false", // Disable FormSubmit recaptcha screen
    ...
    .then(data => {
      console.log("FormSubmit Success Response:", data);
      
      // FormSubmit returns HTTP 200 OK even for unactivated forms/errors, check data.success property
      if (data.success === "false" || data.success === false) {
        throw new Error(data.message || "This form needs activation.");
      }
    ...
    .catch(error => {
      console.error("FormSubmit Error Response:", error);
      
      btn.innerHTML = `<span>Transmission Failed</span> <i data-lucide="alert-triangle"></i>`;
      lucide.createIcons();
      btn.style.background = 'linear-gradient(135deg, #ef4444, #dc2626)'; // Danger red
      
      formStatus.style.color = '#ef4444';
      formStatus.textContent = `Could not transmit message: ${error.message || 'Network error'}. Please try emailing directly to mondalrohan741@gmail.com.`;
